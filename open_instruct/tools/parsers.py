...
if importlib.util.find_spec("vllm.tool_parsers") is not None:
    from vllm.tool_parsers import ToolParser as VllmNativeToolParser

    _VLLM_TOOL_PARSER_BASE = "vllm.tool_parsers"
else:
    from vllm.entrypoints.openai.tool_parsers import ToolParser as VllmNativeToolParser

    _VLLM_TOOL_PARSER_BASE = "vllm.entrypoints.openai.tool_parsers"
...
class VllmParserConfig:
    ...
    import_path: str
    """Import path for the parser class (e.g., '<base>.hermes_tool_parser:Hermes2ProToolParser')."""
    output_template: str
...
VLLM_PARSERS: dict[str, VllmParserConfig] = {
    ...
    "vllm_hermes": VllmParserConfig(
        import_path=f"{_VLLM_TOOL_PARSER_BASE}.hermes_tool_parser:Hermes2ProToolParser",
        output_template="<|im_start|>tool\n<tool_response>\n{}\n</tool_response>\n<|im_end|>\n",
        output_postfix="<|im_start|>assistant\n",
    ),
    # Llama 3.x JSON style
    "vllm_llama3_json": VllmParserConfig(
        import_path=f"{_VLLM_TOOL_PARSER_BASE}.llama_tool_parser:Llama3JsonToolParser",
    ...
    "vllm_olmo3": VllmParserConfig(
        import_path=f"{_VLLM_TOOL_PARSER_BASE}.olmo3_tool_parser:Olmo3PythonicToolParser",
